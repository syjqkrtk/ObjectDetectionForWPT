...
def point_in_shape(x,y,human_in_danger,xx,yy):
    #print(vector)
    ...
    if (rect[0] <= x[1] and rect[2] >= x[1]) and (rect[1] <= y[1] and rect[3] >= y[1]):
        return 1
...
def vis_one_image(
    ...
    ax.axis('off')
    fig.add_axes(ax)
    c = plt.Circle((xx,yy), 10, color='r', fill=False)
    #e = Ellipse(xy=(xx,yy+30), width=60, height=100, angle=0, fill=False, color='b')
    e = plt.Rectangle(xy=(xx-50,yy-20), width=100, height=100, angle=0, fill=False, color='b')
    ax.add_patch(c)
    ax.add_patch(e)
    ax.imshow(im)
    ...
    if wait_to_stop:
        fig.canvas.draw()
        fig2 = np.array(fig.canvas.renderer._renderer)[:,:,:-1]
        fig2 = fig2[:,:,::-1]
        #print(np.shape(fig2))
        #print(fig2)
        #cv2.imshow(im_name,fig2)
        #cv2.waitKey(1)
        return human_in_danger, fig2
        ...
        if keypoints is not None and len(keypoints) > i:
            ...
            for l in range(len(kp_lines)):
                ...
                if kps[2, i1] > kp_thresh and kps[2, i2] > kp_thresh:
                    ...
                    y = [kps[1, i1], kps[1, i2]]
                    line = ax.plot(x, y)
                    human_in_danger = point_in_shape(x,y,human_in_danger,xx,yy)
