# -*- coding: utf-8 -*-
"""
Created on Mon Dec 10 13:53:51 2018
...
def visualize_boxes_and_labels_on_image_array(
  ...
  Args:
    ...
    skip_labels: whether to skip label when drawing a single detection1
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    obj_y_min_tmp = boxes[i][0] * frame_height
    obj_x_min_tmp = boxes[i][1] * frame_width
    obj_y_max_tmp = boxes[i][2] * frame_height
    obj_x_max_tmp = boxes[i][3] * frame_width
           
    #print(iot_x_center, iot_y_center, iot_num, iot_x_center[0])
    temp, human_in_danger_now = point_in_shape([obj_x_min_tmp,obj_x_max_tmp],[obj_y_min_tmp,obj_y_max_tmp],human_in_danger,xx,yy)
    
    if scores is None or scores[i] > min_score_thresh:
        box = tuple(boxes[i].tolist()) # 조건을 만족하는 부분만 박스 추가
  ...
  cv2.circle(img=image, center=(xx, yy), radius=10, color=(0,0,255), thickness=6, lineType=8, shift=0)
 
  cv2.rectangle(image, (xx-100,yy-100), (xx+100,yy+100), (255,0,0), 6)
  cv2.rectangle(image, (xx-100,yy-100), (xx+100,yy+100), (255,0,0), 6)
  
  return image, human_in_danger
